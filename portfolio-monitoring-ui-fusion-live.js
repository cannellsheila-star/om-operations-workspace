(() => {
  const VERSION = "portfolio-live-fusion-station-range-v1";
  ...
  async function loadFusionOverview() {
    ...
    try {
      ...
      // The portfolio response has daily energy and status but not instantaneous
      // PV power. Load each mapped system's station-history endpoint sequentially
      // (never the throttled device endpoints) and enrich the matching row as it arrives.
      const rangeTo = Date.now();
      const rangeFrom = rangeTo - 24 * 60 * 60 * 1000;
      for (const system of workspaceSystems().filter(isFusionSystem)) {
        const systemCodes = fusionCodesFor(system, portfolio);
        const detail = store.detailCache.get(system.id);
        if (!systemCodes.length || !detail) continue;
        try {
          const rangeResponse = await nativeFetch(`/api/fusionsolar-range?stationCodes=${encodeURIComponent(systemCodes.join(","))}&from=${rangeFrom}&to=${rangeTo}`, { cache: "no-store", headers });
          const range = await rangeResponse.json().catch(() => ({}));
          const latest = [...(range.power || [])].reverse().find((row) => num(row?.pvKw) !== null);
          ...
          // Keep the verified station status and daily energy visible if one
          // system's optional hourly-history request is temporarily unavailable.
        }
      }
    } catch (error) {
