(() => {
  ...
  function savingsRate(system) {
    const structure=parseTariffStructure(system);
    if(structure.type==="flat"&&num(structure.components?.energyPerKwh)!==null)return num(structure.components.energyPerKwh);
    if(!structure.type)return parsePerKwh(system.utilityTariff);
    return null;
  }
  function savingsRequest(rows) {
    return {period:{from:reportState.from,to:reportState.to},sites:rows.map(({system,history})=>{
      const structure=parseTariffStructure(system);
      return {systemId:system.id,monitoring:history?.totals||{},tariff:{ratePerKwh:savingsRate(system),exportRatePerKwh:num(structure.exportRate)},contract:{ppaRatePerKwh:parsePerKwh(system.ppaRate),eassMonthly:parseMoneyValue(system.eassRate)}};
    })};
  }
  async function calculateSavings(rows) {
    try {
      ...
      return new Map(rows.map((row)=>[String(row.system.id),{status:"Savings calculation unavailable"}]));
    }
  }
  function financials(result) {
    if(!result)return null;
    return {avoided:num(result.avoidedUtility),exportCredit:num(result.exportCredit),selfConsumedKwh:num(result.energyOffsetKwh),status:result.status||"Savings calculation unavailable",exact:num(result.netSaving)!==null,ppa:num(result.ppaCharge),eass:num(result.eassCharge),net:num(result.netSaving)};
  }
  ...
  async function loadRows(force=false) {
    ...
    try{await ensureFeeds();const list=selectedSystems();const rows=[];for(let i=0;i<list.length;i+=1){const system=list[i],live=liveSnapshot(system);reportState.progress=`Loading ${i+1} of ${list.length}: ${system.name}`;renderReports(false);let history=null,error="";if(live.linked){try{history=await loadPeriod(system);}catch(e){error=e?.message||"History unavailable";}}rows.push({system,live,history,error,tickets:reportTickets(system)});if(Array.isArray(system.fusionSolarPlants)&&system.fusionSolarPlants.length)await new Promise((r)=>setTimeout(r,180));}reportState.progress="Calculating savings";renderReports(false);const savingResults=await calculateSavings(rows);rows.forEach((row)=>{row.savings=financials(savingResults.get(String(row.system.id)));});reportState.rows=rows;reportState.loadedKey=currentKey;}catch(e){reportState.error=e?.message||"Report data could not be loaded.";}finally{reportState.loading=false;reportState.progress="";renderReports(false);}
  }
